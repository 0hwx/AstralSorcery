...
public class GuiProgressionClusterRenderer {
    ...
    private static final BindableResource frameBlank = AssetLibrary
        .loadTexture(AssetLoader.TextureLocation.GUI, "frameBlank");
    private static final BindableResource frameWooden = AssetLibrary
        .loadTexture(AssetLoader.TextureLocation.GUI, "frameWooden");
    ...
    public GuiProgressionClusterRenderer(ResearchProgression progression, int guiHeight, int guiWidth, int guiLeft,
        int guiTop) {
            ...
            for (Rectangle r : clickableNodes.keySet()) {
                if (r.contains(p)) {
                    ...
                    Minecraft.getMinecraft()
                        .displayGuiScreen(new GuiJournalPages(parent, clicked));
                }
            }
        }
    }
            ...
            for (Rectangle r : clickableNodes.keySet()) {
                if (r.contains(mousePoint)) {
                    ...
                    GL11.glScaled(
                        ...
                        partSizeHandler.getScalingFactor(),
                        partSizeHandler.getScalingFactor());
                    String name = clickableNodes.get(r)
                        .getUnLocalizedName();
                    name = I18n.format(name);
                    RenderingUtils.renderTooltip(
                        0,
                        0,
                        Lists.newArrayList(name),
                        new Color(0x00100033),
    ...
    private void drawNodesAndConnections(float zLevel) {
        ...
        for (ResearchNode node : progression.getResearchNodes()) {
            ...
            double lX = midX
                + (absX * (partSizeHandler.getZoomedWHNode() + partSizeHandler.getZoomedSpaceBetweenNodes()));
            double lZ = midY
                + (absZ * (partSizeHandler.getZoomedWHNode() + partSizeHandler.getZoomedSpaceBetweenNodes()));
    ...
    private void renderNodeToGUI(ResearchNode node, double lowerPosX, double lowerPosY, float zLevel) {
        ...
        if (partSizeHandler.getScalingFactor() >= 0.7) {
            clickableNodes.put(
                new Rectangle(
                    ...
                    MathHelper.floor_double(zoomedWH),
                    MathHelper.floor_double(zoomedWH)),
                node);
        }
        ...
        switch (node.getRenderType()) {
            ...
            case TEXTURE:
                ...
                // GL11.glDisable(GL11.GL_ALPHA_TEST);
                // GL11.glColor4f(renderLoopBrFactor, renderLoopBrFactor, renderLoopBrFactor, renderLoopBrFactor);
                BindableResource tex = node.getTexture()
                    .resolve();
                tex.bind();
            ...
            case TEXTURE_SPRITE:
                ...
                GL11.glColor4f(renderLoopBrFactor, renderLoopBrFactor, renderLoopBrFactor, renderLoopBrFactor);
                SpriteSheetResource res = node.getSpriteTexture()
                    .resolveSprite();
                res.getResource()
                    .bind();
                ...
                tess.addVertexWithUV(
                    ...
                    zoomedWH,
                    zLevel,
                    uvTexture.key + res.getULength(),
                    uvTexture.value + res.getVLength());
                ...
                // vb.pos(zoomedWH, zoomedWH, zLevel).tex(uvTexture.key + res.getULength(), uvTexture.value +
                // res.getVLength()).endVertex();
    ...
    private void renderConnectionLines(ResearchNode node, double lowerPosX, double lowerPosY, double midX, double midY,
        float zLevel) {
        double xAdd = (lowerPosX - (this.mousePointScaled.getScaledPosX() - partSizeHandler.widthToBorder))
            + partSizeHandler.getZoomedWHNode() / 2;
        double yAdd = (lowerPosY - (this.mousePointScaled.getScaledPosY() - partSizeHandler.heightToBorder))
            + partSizeHandler.getZoomedWHNode() / 2;
    ...
    private void renderConnection(ResearchNode to, double fromX, double fromY, double midX, double midY, float zLevel) {
        double relToX = midX
            + (to.renderPosX * (partSizeHandler.getZoomedWHNode() + partSizeHandler.getZoomedSpaceBetweenNodes()));
        double relToY = midY
            + (to.renderPosZ * (partSizeHandler.getZoomedWHNode() + partSizeHandler.getZoomedSpaceBetweenNodes()));
        double targetXOffset = (relToX - (this.mousePointScaled.getScaledPosX() - partSizeHandler.widthToBorder))
            + (partSizeHandler.getZoomedWHNode() / 2);
        double targetYOffset = (relToY - (this.mousePointScaled.getScaledPosY() - partSizeHandler.heightToBorder))
            + (partSizeHandler.getZoomedWHNode() / 2);
