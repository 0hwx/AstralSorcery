...
public class GuiJournalPerkMap extends GuiScreenJournal {
    ...
    private void drawUnlockEffects() {
        ...
        Iterator<ConstellationPerks> iterator = unlockPlayMap.keySet()
            .iterator();
        GL11.glEnable(GL11.GL_BLEND);
        Blending.DEFAULT.apply();
        ...
        while (iterator.hasNext()) {
            ...
            Vector3 starVec = offset.clone()
                .addX(starX * whBetweenStars - whStar)
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // vb.pos(pos.getX(), pos.getY(), pos.getZ()).tex(frameUV.key + uLength * u, frameUV.value + vLength *
                // v).endVertex();
            }
            tess.draw();
        }
    ...
    private void drawPerkMap(ConstellationPerkMap mapToDisplay, Point mouse) {
        ...
        Map<Rectangle, ConstellationPerks> rects = drawStars(
            mapToDisplay.getPerks(),
            offsetX,
            offsetY,
            whStar,
            whBetweenStars);
        ...
        for (Rectangle r : rects.keySet()) {
            if (r.contains(mouse)) {
                ...
                ConstellationPerk perk = rects.get(r)
                    .getSingleInstance();
                ...
                if (prog.hasPerkUnlocked(perk)) {
                    int unlock = prog.getAppliedPerks()
                        .get(perk);
                    if (unlock > 0) {
        ...
        for (ConstellationPerkMap.Dependency dep : mapToDisplay.getPerkDependencies()) {
            ...
            if (from != null && to != null) {
                ...
                Vector3 fromStar = new Vector3(
                    offset.getX() + from.x * whBetweenStars,
                    offset.getY() + from.y * whBetweenStars,
                    offset.getZ());
                Vector3 toStar = new Vector3(
                    offset.getX() + to.x * whBetweenStars,
                    offset.getY() + to.y * whBetweenStars,
                    offset.getZ());
    ...
    private Map<Rectangle, ConstellationPerks> drawStars(Map<ConstellationPerks, ConstellationPerkMap.Position> perks,
        double offsetX, double offsetY, double whStar, double whBetweenStars) {
