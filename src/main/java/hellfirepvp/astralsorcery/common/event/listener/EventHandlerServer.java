...
public class EventHandlerServer {
    ...
    public static TimeoutListContainer<PlayerWrapperContainer, Integer> perkCooldowns = new TimeoutListContainer<>(
        new ConstellationPerks.PerkTimeoutHandler(),
        TickEvent.Type.SERVER);
    public static TimeoutListContainer<PlayerWrapperContainer, Integer> perkCooldownsClient = new TimeoutListContainer<>(
        new ConstellationPerks.PerkTimeoutHandler(),
    ...
    public static TickTokenizedMap<WorldBlockPos, TickTokenizedMap.SimpleTickToken<Double>> spawnDenyRegions = new TickTokenizedMap<>(
        TickEvent.Type.SERVER);
    // public static TimeoutListContainer<EntityPlayer, Integer> perkCooldowns = new TimeoutListContainer<EntityPlayer,
    // Integer>(new ConstellationPerks.PerkTimeoutHandler(), TickEvent.Type.SERVER);
    ...
    // List<EntityLivingBase> others = entity.worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
    // entity.boundingBox.expand(3,3,3)); //, (e) -> !e.isDead && e != entity);
    // for (EntityLivingBase lb : others) {
    // lb.setFire(16);
    // lb.knockBack(entity, 2F, lb.posX - entity.posX, lb.posZ - entity.posZ);
    // }
    // PktParticleEvent ev = new PktParticleEvent(PktParticleEvent.ParticleEventType.PHOENIX_PROC, new
    // Vector3(entity.posX, entity.posY, entity.posZ));
    ...
    // boolean mayDeny = Config.doesMobSpawnDenyDenyEverything || toTest.isCreatureType(EnumCreatureType.monster,
    // false);
    // if(mayDeny) {
    // for (Map.Entry<WorldBlockPos, TickTokenizedMap.SimpleTickToken<Double>> entry : spawnDenyRegions.entrySet()) {
    // if(!entry.getKey().getWorld().equals(toTest.worldObj)) continue;
    ...
    // WorldCacheManager.getInstance().doSave(event.world);
    // }
    //
    // @SubscribeEvent
    ...
    // if(oldState.equals(BlockCustomOre.OreType.ROCK_CRYSTAL))
    // {//oldState.getValue(BlockCustomOre.ORE_TYPE).equals(BlockCustomOre.OreType.ROCK_CRYSTAL)) {
    // ((RockCrystalBuffer) WorldCacheManager.getOrLoadData(event.getWorld(),
    // WorldCacheManager.SaveKey.ROCK_CRYSTAL)).removeOre(event.getPos());
    // }
    // }
    // }
    //
    ...
    // WorldNetworkHandler.getNetworkHandler(event.world).informTablePlacement(at);
    // }
    // }
    //
    // @SubscribeEvent(priority = EventPriority.LOWEST)
    ...
    // WorldNetworkHandler.getNetworkHandler(event.world).informTableRemoval(at);
    // }
    // }
    //
    ...
    // FMLCommonHandler.instance().firePlayerSmeltedEvent(event.harvester, furnaced);
    ...
    public void onHarvestSpeedCheck(net.minecraftforge.event.entity.player.PlayerEvent.BreakSpeed event) {
        ...
        if (harvester != null) {
            PlayerProgress prog = ResearchManager
                .getProgress(harvester, harvester.getEntityWorld().isRemote ? Side.CLIENT : Side.SERVER);
            if (prog != null) {
                Map<ConstellationPerk, Integer> perks = prog.getAppliedPerks();
                for (ConstellationPerk perk : perks.keySet()) {
                    if (!prog.isPerkActive(perk)) continue;
                    if (perk.mayExecute(ConstellationPerk.Target.PLAYER_HARVEST_SPEED)) {
                        ...
                        event.newSpeed = (perk
                            .onHarvestSpeed(harvester, event.block, (p == null || y < 0) ? null : p, event.newSpeed));
                    }
                }
            }
        }
            ...
            if (prog != null) {
                ...
                for (ConstellationPerk perk : perks.keySet()) {
                    ...
                    if (perk.mayExecute(ConstellationPerk.Target.PLAYER_HARVEST_TYPE)) {
                        if (perk.onCanHarvest(harvester, harvester.getHeldItem(), event.block, event.success)) {
                            event.success = true;
                        }
                    }
                }
    ...
    // @SubscribeEvent
    // public void onContainerOpen(PlayerOpenContainerEvent event) {
    // if(event.getContainer() instanceof ContainerWorkbench && !event.entityPlayer.worldObj.isRemote &&
    ...
    private void phoenixEffects(EntityLivingBase entity, int level) {
        ...
        entity.addPotionEffect(new PotionEffect(Potion.fireResistance.getId(), 500, 1));
        ...
        PacketChannel.CHANNEL.sendToAllAround(
            ev,
            PacketChannel.pointFromPos(entity.worldObj, (int) entity.posX, (int) entity.posY, (int) entity.posZ, 32));
    ...
    public void onRightClickLast(PlayerInteractEvent event) {
        ...
        if (!event.world.isRemote) {
            Block interacted = event.world.getBlock(event.x, event.y, event.z);;
            if (interacted instanceof BlockWorkbench) {
                PktCraftingTableFix fix = new PktCraftingTableFix(event.x, event.y, event.z);
                PacketChannel.CHANNEL.sendTo(fix, (EntityPlayerMP) event.entityPlayer);
            }
        }
    }
    ...
    public void onChange(BlockModifyEvent event) {
        ...
        // Block oldState = event.getOldBlock();
        // if (oldState.equals(BlockCustomOre.ORE_TYPE)
    ...
    public void onBreak(BlockEvent.BreakEvent event) {
        ...
        if (broken.equals(Blocks.crafting_table)) {
            WorldNetworkHandler.getNetworkHandler(event.world)
                .informTableRemoval(at);
        }

        // ItemStack active = event.getPlayer().getHeldItem();
        // WandAugment found = null;
        // if (active != null && active.getItem() instanceof ItemWand) {
