...
public class Config {
    ...
    @Sync
    public static float evorsioEffectChance = 0.25F;
    @Sync
        ...
        for (ConfigDataAdapter<?> cfg : dataAdapters) {
            if (cfg.getDataFileName()
                ...
                throw new IllegalArgumentException(
                    "Duplicate DataRegistry names! " + cfg.getDataFileName() + " - " + dataAdapter.getDataFileName());
            }
        }
        ...
        for (String str : out) {
            if (cfg.appendDataSet(str) == null) {
                AstralSorcery.log.warn(
                    "[AstralSorcery] Skipped Entry '" + str
                        + "' for registry "
                        + cfg.getDataFileName()
                        + "! Invalid format!");
            }
    ...
    private static void fillDimGenWhitelist(String[] dimGenWhitelist) {
        ...
        for (String s : dimGenWhitelist) {
            ...
            try {
                ...
                AstralSorcery.log.warn(
                    "[AstralSorcery] Error while reading config entry 'worldGenWhitelist': " + s + " is not a number!");
            }
        }
    ...
    private static void fillWeakSkyRenders(String[] weakSkyRenders) {
        ...
        for (String s : weakSkyRenders) {
            ...
            try {
                ...
                AstralSorcery.log.warn(
                    "[AstralSorcery] Error while reading config entry 'weakSkyRenders': " + s + " is not a number!");
            }
        }
    ...
    private static void fillWhitelistIDs(String[] dimWhitelist) {
        ...
        for (String s : dimWhitelist) {
            ...
            try {
                ...
                AstralSorcery.log.warn(
                    "[AstralSorcery] Error while reading config entry 'skySupportedDimensions': " + s
                        + " is not a number!");
            }
        }
