...
public class ConstellationPerkLevelManager {
     ...
     * return Math.min(1F, ((float) levelsRequired[perks] - have) / ((float) levelsRequired[perks] -
     * levelsRequired[perks - 1]));
     * }
    ...
    public static ConfigEntry getLevelConfigurations() {
        return new ConfigEntry(ConfigEntry.Section.PERK_LEVELS, "levels") {
            ...
            public void loadFromConfig(Configuration cfg) {
                ...
                levelBuf[0] = cfg.getInt(
                    getKey() + "_Level_1",
                    getConfigurationSection(),
                    15,
                    1,
                    Integer.MAX_VALUE,
                    "Defines the amount of Alignment needed for level: 1");
                levelBuf[1] = cfg.getInt(
                    getKey() + "_Level_2",
                    getConfigurationSection(),
                    45,
                    2,
                    Integer.MAX_VALUE,
                    "Defines the amount of Alignment needed for level: 2");
                levelBuf[2] = cfg.getInt(
                    getKey() + "_Level_3",
                    getConfigurationSection(),
                    85,
                    3,
                    Integer.MAX_VALUE,
                    "Defines the amount of Alignment needed for level: 3");
                levelBuf[3] = cfg.getInt(
                    getKey() + "_Level_4",
                    getConfigurationSection(),
                    130,
                    4,
                    Integer.MAX_VALUE,
                    "Defines the amount of Alignment needed for level: 4");
                levelBuf[4] = cfg.getInt(
                    getKey() + "_Level_5",
                    getConfigurationSection(),
                    180,
                    5,
                    Integer.MAX_VALUE,
                    "Defines the amount of Alignment needed for level: 5");
                levelBuf[5] = cfg.getInt(
                    getKey() + "_Level_6",
                    getConfigurationSection(),
                    245,
                    6,
                    Integer.MAX_VALUE,
                    "(Unused YET) Defines the amount of Alignment needed for level: 6");
                levelBuf[6] = cfg.getInt(
                    getKey() + "_Level_7",
                    getConfigurationSection(),
                    325,
                    7,
                    Integer.MAX_VALUE,
                    "(Unused YET) Defines the amount of Alignment needed for level: 7");
                levelBuf[7] = cfg.getInt(
                    getKey() + "_Level_8",
                    getConfigurationSection(),
                    400,
                    8,
                    Integer.MAX_VALUE,
                    "(Unused YET) Defines the amount of Alignment needed for level: 8");
                for (int i = 0; i < 7; i++) {
                    int prev = levelBuf[i];
                    int next = levelBuf[i + 1];
                    if (prev >= next) {
                        throw new IllegalStateException(
                            "Illegal AstralSorcery config state: Alignment required for level " + (i + 2)
                                + " is lower than alignment needed for level "
                                + (i + 1));
                    }
                }
