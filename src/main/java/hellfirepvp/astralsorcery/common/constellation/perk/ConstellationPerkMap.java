...
public class ConstellationPerkMap {
    ...
    public ConstellationPerkMap addPerk(ConstellationPerks perk, PerkOrder obtainOrder, int x, int y,
        ConstellationPerks... dependencies) {
        if (perks.containsKey(perk)) {
            AstralSorcery.log.warn(
                "[AstralSorcery] Tried to register the same perk (id=" + perk.ordinal()
                    + ") twice to a perk map! Skipping registration...");
            return this;
        }
        ...
        for (ConstellationPerks from : dependencies) {
            if (perks.containsKey(from)) {
                ...
                for (Dependency c : perkDependencies) {
                    if ((c.from.equals(from) && c.to.equals(perk)) || (c.to.equals(from) && c.from.equals(perk))) {
                        found = true;
                    }
                }
                if (!found) {
                    ...
                    AstralSorcery.log.warn(
                        "[AstralSorcery] Tried to register a perk-dependency that already exists! Skipping registration...");
                }
            } else {
                AstralSorcery.log.warn(
                    "[AstralSorcery] Tried to register a perk-dependency from a non-registered perk! Skipping registration...");
            }
        }
        return this;
    }
    ...
    @Deprecated
    public List<ConstellationPerks> getAvailablePerksFor(EntityPlayer player, PerkOrder availableOrder,
        boolean client) {
