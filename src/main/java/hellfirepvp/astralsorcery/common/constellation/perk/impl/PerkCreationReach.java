...
public class PerkCreationReach extends ConstellationPerk {
            ...
            if (!isCooldownActiveForPlayer(player)) {
                if (player instanceof EntityPlayerMP) {
                    double reach = Math.max(
                        5D,
                        ((EntityPlayerMP) player).theItemInWorldManager.getBlockReachDistance() + reachModifier);
    ...
    public void onTimeout(EntityPlayer player) {
        if (player instanceof EntityPlayerMP) {
            double reach = Math
                .max(5D, ((EntityPlayerMP) player).theItemInWorldManager.getBlockReachDistance() - reachModifier);
            ((EntityPlayerMP) player).theItemInWorldManager.setBlockReachDistance(reach);
    ...
    public static void updateReach(boolean apply, float modifierIn) {
        ...
        if (pl != null && ctrl != null) {
            ...
            ExtendedChainingPlayerController ctr = (ExtendedChainingPlayerController) Minecraft
    ...
    public void loadFromConfig(Configuration cfg) {
        reachModifier = cfg.getFloat(
            getKey() + "ReachModifier",
            getConfigurationSection(),
            2.5F,
            0F,
            200F,
            "Sets the reach modifier that gets applied when the player has this perk. (Too high values might cause issues.)");
    }
