...
public abstract class CEffectPositionListGen<T extends CEffectPositionListGen.CEffectGenListEntry>
    ...
    public CEffectPositionListGen(@Nullable ILocatable origin, IWeakConstellation constellation, String cfgName,
        int searchRange, int maxCount, Verifier verifier, Function<BlockPos, T> emptyElementProvider) {
    ...
    public boolean findNewPosition(World world, BlockPos pos) {
        if (maxCount > elements.size()) {
            ...
            if (MiscUtils.isChunkLoaded(world, new ChunkCoordIntPair(at.chunkX(), at.chunkZ()))
                && verifier.isValid(world, at)
                && !containsElementAt(at)) {
    ...
    // FIXME SOMEDAY the read version is not world sensitive -> fertilitas reeds?... they need world dependent
    // checking..
