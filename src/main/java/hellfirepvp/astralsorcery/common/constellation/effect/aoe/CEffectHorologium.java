...
public class CEffectHorologium extends CEffectPositionList {
    ...
    public CEffectHorologium(@Nullable ILocatable origin) {
        super(
            ...
            searchRange,
            maxCount,
            (world, pos) -> TileAccelerationBlacklist
                .canAccelerate(world.getTileEntity(pos.getX(), pos.getY(), pos.getZ())));
    }
                ...
                if (TileAccelerationBlacklist.canAccelerate(te)) { // Does != null && instanceof ITickable check.
                    ...
                    try {
                        ...
                        AstralSorcery.log.warn(
                            "[AstralSorcery] Couldn't accelerate TileEntity " + te.getClass()
                                .getName() + " properly.");
                        AstralSorcery.log.warn(
                            "[AstralSorcery] Temporarily blacklisting that class. Consider adding that to the blacklist if it persists?");
                        exc.printStackTrace();
                    }
                } else {
                    removeElement(entry);
