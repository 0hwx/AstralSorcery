...
// );
// Vector3 dir = particlePos.clone().subtract(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() +
/// 0.5).normalize().divide(-30);
// EntityFXFacingParticle p = EffectHelper.genericFlareParticle(particlePos.getX(), particlePos.getY(),
/// particlePos.getZ());
// p.motion(dir.getX(), dir.getY(), dir.getZ()).setAlphaMultiplier(1F).setMaxAge(rand.nextInt(40) + 20);
// p.enableAlphaFade(EntityComplexFX.AlphaFunction.FADE_OUT).scale(0.2F + rand.nextFloat() * 0.1F).setColor(new
/// Color(29, 123, 59));
// }
// }
//
// @Override
// public boolean playMainEffect(World world, BlockPos pos, float percStrength, boolean mayDoTraitEffect, @Nullable
/// IMinorConstellation possibleTraitEffect) {
...
// if(entry != null) {
// entry.counter++;
// PktParticleEvent ev = new PktParticleEvent(PktParticleEvent.ParticleEventType.CE_SPAWN_PREPARE_EFFECTS,
/// entry.getPos());
...
// }
//
// List<EntityLivingBase> entities = world.getEntitiesWithinAABB(EntityLivingBase.class, proximityCheckBox.offset(pos),
/// e -> e != null && !e.isDead);
...
// searchRange = cfg.getInt(getKey() + "Range", getConfigurationSection(), searchRange, 1, 32, "Defines the radius (in
/// blocks) in which the ritual will search for potential spawn points for entities.");
// enabled = cfg.getBoolean(getKey() + "Enabled", getConfigurationSection(), enabled, "Set to false to disable this
/// ConstellationEffect.");
// potencyMultiplier = cfg.getFloat(getKey() + "PotencyMultiplier", getConfigurationSection(), potencyMultiplier, 0.01F,
/// 100F, "Set the potency multiplier for this ritual effect. Will affect all ritual effects and their efficiency.");
//
// selectChance = cfg.getFloat(getKey() + "SpawnSearchChance", getConfigurationSection(), selectChance, 0F, 1F, "Defines
/// the per-tick chance that a new position for a entity-spawn will be searched for.");
// proximityAmount = cfg.getInt(getKey() + "ProximityCheck", getConfigurationSection(), proximityAmount, 1, 256,
/// "Defines the threshold at which the ritual will stop spawning mobs. If there are more or equal amount of mobs near
/// this ritual, the ritual will not spawn more mobs. Mainly to reduce potential server lag.");
// }
//
// @SideOnly(Side.CLIENT)
