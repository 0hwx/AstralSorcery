...
public class TileRitualPedestal extends TileReceiverBaseInventory {
        ...
        if (worldObj.isRemote && working) {
            ...
            if (shouldDoAdditionalEffects() && !isDay) {
                if (EffectHandler.STATIC_EFFECT_RAND.nextInt(chance * 2) == 0) {
                    ...
                    from.setY(
                        yCoord - 0.6
                            + 1 * EffectHandler.STATIC_EFFECT_RAND.nextFloat()
                                * (EffectHandler.STATIC_EFFECT_RAND.nextBoolean() ? 1 : -1));
                    EffectLightbeam lightbeam = EffectHandler.getInstance()
                        .lightbeam(
                            from.clone()
                                .addY(5 + EffectHandler.STATIC_EFFECT_RAND.nextInt(3)),
                            from,
                            1.3F);
            ...
            if (crystal != null && crystal.getItem() != null && crystal.getItem() instanceof ItemTunedCrystalBase) {
            ...
            for (BlockPos expMirror : offsetMirrorPositions) {
                if (ticksExisted % 32 == 0) {
                    ...
                    Vector3 to = new Vector3(this).add(expMirror.getX(), expMirror.getY(), expMirror.getZ())
                        .add(0.5, 0.5, 0.5);
    ...
    public TransmissionReceiverRitualPedestal getUpdateCache() {
        ...
        if (cachePedestal != null) {
            if (!cachePedestal.getPos()
                .equals(new BlockPos(xCoord, yCoord, zCoord))) {
                cachePedestal = null;
            }
        }
    ...
    protected void onInventoryChanged(int slotChanged) {
        if (!worldObj.isRemote) {
            ...
            if (in != null && in.getItem() != null && in.getItem() instanceof ItemTunedCrystalBase) {
                ...
                if (recNode != null) {
                    ...
                    AstralSorcery.log.warn(
                        "[AstralSorcery] Tried to find receiver node at dimId=" + worldObj.provider.dimensionId
                            + " pos="
    ...
    public static class TransmissionReceiverRitualPedestal extends SimpleTransmissionReceiver {
        ...
        public void update(World world) {
            ...
            if (channeling != null && properties != null && hasMultiblock) {
                if (ce == null) {
                     ...
                     * tw = new TreeCaptureHelper.TreeWatcher(world.provider.dimensionId, getPos(),
                ...
                if (doesSeeSky) {
                    ...
                    double collect = 0;
                    if (handle != null) {
                        collect = perc * CrystalCalculations.getCollectionAmt(
                            properties,
        ...
        public void readFromNBT(NBTTagCompound compound) {
            ...
            if (c != null && !(c instanceof IWeakConstellation)) {
                AstralSorcery.log.warn(
                    "[AstralSorcery] Tried to load RitualPedestal from NBT with a non-Major constellation as effect. Ignoring constellation...");
            ...
            if (c != null && !(c instanceof IMinorConstellation)) {
                AstralSorcery.log.warn(
                    "[AstralSorcery] Tried to load RitualPedestal from NBT with a non-Minor constellation as trait. Ignoring constellation...");
                AstralSorcery.log.warn("[AstralSorcery] Block affected is at " + getPos());
            } else if (c == null) {
                trait = null;
            } else {
