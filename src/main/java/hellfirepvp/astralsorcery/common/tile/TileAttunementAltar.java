...
public class TileAttunementAltar extends TileEntityTick {
            ...
            if (activeFound != null) {
                ...
                if (mode == 0) {
                    ...
                    if (!(activeEntity instanceof EntityPlayer) || activeEntity.isDead
                        || new Vector3(activeEntity).distance(new Vector3(this)) > 4) {
                    ...
                    if (activeEntity.isDead || !(activeEntity instanceof EntityItem)
                        ...
                        if (serverSyncAttTick >= TICKS_CRYSTAL_ATTUNEMENT) {
                            ...
                            CrystalProperties
                                .applyCrystalProperties(tunedStack, CrystalProperties.getCrystalProperties(current));
                            ...
                            EntityFlare.spawnAmbient(
                                ...
                                new Vector3(this).add(-3 + rand.nextFloat() * 7, 0.6, -3 + rand.nextFloat() * 7));
                            SoundHelper
                                .playSoundAround(Sounds.craftFinish, worldObj, xCoord, yCoord, zCoord, 2.5F, 1.6F);
                        }
                    }
                }
            }
        ...
        if (activeFound instanceof IMajorConstellation) {
            ...
            if (!players.isEmpty()) {
                EntityPlayerMP pl = EntityUtils.selectClosest(
                    players,
                    (player) -> thisVec.distanceSquared(new BlockPos(player).getPositionVector()));// player.getPositionVector()));
    ...
    private void matchActiveConstellation() {
        ...
        for (BlockPos pos : positions) {
            ...
            if (!state.equals(BlocksAS.attunementRelay) && !state.equals(BlocksAS.attunementAltar)) {
                valid = false;
            }
        ...
        if (activeFound == null) {
            ...
            if (player != null && player.getDistanceSq(xCoord, yCoord, zCoord) <= 250) {
                ...
                if (player.getHeldItem() != null && player.getHeldItem()
                    .getItem() instanceof ItemConstellationPaper) {
                ...
                if (held == null && player.getHeldItem() != null
                    && player.getHeldItem()
                        .getItem() instanceof ItemConstellationPaper) {
                    IConstellation cst = ItemConstellationPaper.getConstellation(player.getHeldItem());
                    if (cst != null && cst instanceof IWeakConstellation) {
    ...
    private List<Tuple<BlockPos, BlockPos>> translateConnectionPositions(IConstellation cst) {
        ...
        for (StarConnection c : cst.getStarConnections()) {
            ...
            offsetPositions.add(
                new Tuple<>(
                    new BlockPos(from.x / 2 - 7, 0, from.y / 2 - 7).add(pos),
                    new BlockPos(to.x / 2 - 7, 0, to.y / 2 - 7).add(pos)));
        }
