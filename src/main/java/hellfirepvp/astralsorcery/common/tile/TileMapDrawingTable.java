...
public class TileMapDrawingTable extends TileSkybound {
    ...
    public void tick() {
        ...
        if (worldObj.isRemote) {
            ...
            if (ConstellationSkyHandler.getInstance()
                .isNight(getWorldObj()) && doesSeeSky()
                && slotGlassLens != null
                && slotGlassLens.getItem() != null
                ...
                if (map != null && slotIn != null
                    ...
                    && ((slotIn.isItemEnchantable()
                        ...
                        || (slotIn.getItem() instanceof ItemPotion))) {// &&
                                                                       // PotionUtils.getEffectsFromStack(slotIn).isEmpty())))
                                                                       // {
                    runTick++;
                    if (runTick > RUN_TIME) {
                        if (slotIn.isItemEnchantable()) {
                            ...
                            if (slotGlassLens.attemptDamageItem(1, rand)) {
                                ...
                                // worldObj.playSound(xCoord, yCoord, zCoord, SoundEvents.BLOCK_GLASS_BREAK,
                                // SoundCategory.BLOCKS, rand.nextFloat() * 0.5F + 1F, rand.nextFloat() * 0.2F + 0.8F);
                            }
                        } else if (Items.potionitem.getEffects(slotIn)
                            .isEmpty()) {
                                ...
                                if (rand.nextInt(3) == 0 && slotGlassLens.attemptDamageItem(1, rand)) {
                                    ...
                                    // SoundCategory.BLOCKS, rand.nextFloat() * 0.5F + 1F, rand.nextFloat() * 0.2F +
                                    // 0.8F);
                                }
                            }
                        runTick = 0;
    ...
    public boolean hasParchment() {
        return slotIn != null && slotIn.getItem() != null
            && slotIn.getItem() instanceof ItemCraftingComponent
            && slotIn.getItemDamage() == ItemCraftingComponent.MetaType.PARCHMENT.getMeta()
            && slotIn.stackSize > 0;
    }

    public boolean hasUnengravedGlass() {
        return slotGlassLens != null && slotGlassLens.getItem() != null
            && slotGlassLens.getItem() instanceof ItemInfusedGlass
    ...
    public void tryEngraveGlass(java.util.List<DrawnConstellation> constellations) {
        if (hasParchment() && hasUnengravedGlass()) {
            ...
            PacketChannel.CHANNEL
                .sendToAllAround(ev, PacketChannel.pointFromPos(worldObj, new BlockPos(xCoord, yCoord, zCoord), 16));
        }
    }
