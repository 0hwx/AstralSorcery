...
public class ItemWand extends Item implements ISpecialInteractItem {
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!isSelected)
            isSelected = (entityIn instanceof EntityPlayer) && ((EntityPlayer) entityIn).getHeldItem() == stack;
        if (!worldIn.isRemote && isSelected
            && worldIn.getTotalWorldTime() % 20 == 0
            ...
            for (BlockPos rPos : posList) {
                ...
                if (!(state instanceof BlockCustomOre)) {// || state.getValue(BlockCustomOre.ORE_TYPE) !=
                                                         // BlockCustomOre.OreType.ROCK_CRYSTAL) {
                    buf.removeOre(rPos);
                    continue;
                }
